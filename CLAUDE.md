...
---
## PROJECT STATUS: v3.0.1 (May 2026)

**Release-pipeline repair on top of v3.0.0.** v3.0.0 CI failed (`NETSDK1005` — submodule `PlanViewer.Core` targets only `net8.0` upstream, our release publishes `-f net9.0`); macOS-only assets were uploaded by hand from the dev Mac, Windows shipped nothing. v3.0.1 lands `lib/Directory.Build.props` overriding the dependency's framework set to `net8.0;net9.0;net10.0` (submodule files untouched), `fail-fast: false` on the release matrix so a future macOS hiccup can't kill Windows again, and a re-tag so Windows users on v2.15.1 can auto-update.

**Path B SQL formatter overhaul** — `TSqlFragmentVisitor`-based engine replacing the legacy regex-driven Hogimn formatter. Default-on as of 4h; legacy Hogimn still selectable via Settings → Preview Features → "Use new SQL formatter" (opt-out for one observation week, then deleted). Internals at [docs/FORMATTER-INTERNALS.md](docs/FORMATTER-INTERNALS.md); strategic record at [docs/FORMATTER-OVERHAUL.md](docs/FORMATTER-OVERHAUL.md). Slices 4a–4f-iv landed: visitor skeleton + clause-keyword right-alignment (4a); `SelectStatement` / `QuerySpecification` / subqueries / joins / CTEs / list-wrap / `CASE` / `UNION` / `BooleanBinaryExpression` (4b); `INSERT` / `UPDATE` / `DELETE` / `MERGE` (4c); pre-parse `;WITH` repair (4c-ii); `CREATE/ALTER PROCEDURE` (4d-i); `CREATE/ALTER VIEW` (4d-ii); `CREATE/ALTER FUNCTION` (4d-iii); `CREATE/ALTER TRIGGER` (4d-iv); `CREATE/ALTER TABLE` (4d-v); `BEGIN/END` and `TRY/CATCH` body recursion (4e-i); `IF` and `WHILE` with subquery-in-predicate break-to-block (4e-ii); vertical spacing rule between body siblings (4e-ii-b); `DECLARE` / `DECLARE @t TABLE` / `ROLLBACK TRANSACTION` taste refinements (4e-iii); `PIVOT` / `UNPIVOT` with full-`MaxLineLength` IN-list wrap, `SET @var = (subquery)` break-to-block, bundled fix for bare-QuerySpec trailing-clause drop on TOP/OFFSET/FOR niche features (4f); `QuerySpecification` niche-feature trip-flag retired — TOP inline within SELECT, OFFSET / FOR as their own clause keywords, `EmitGeneratorRaw` no longer used for niche-feature subqueries (4f-ii); `INSERT` / `UPDATE` / `DELETE` `TopRowFilter` trip-fallbacks retired — TOP inline at each statement's lead-keyword body, `EmitTopRowFilter` reused unchanged from 4f-ii (4f-iii); JOIN ON multi-AND/OR col-0 leak retired via synthetic clause scope on BooleanBinaryExpression search conditions — AND/OR right-aligns with ON, single-cond ON keeps the existing inline-or-long-break heuristic (4f-iv); default-on flip — `UseNewFormatter` default flipped from `false` to `true`, legacy Hogimn formatter still selectable via Settings UI as opt-out for one observation week before deletion (4h). `LOOKOUT_USE_NEW_FORMATTER` env var still honored at startup. Falls back to legacy on parse error and surfaces the fallback to the active tab's message area. Suite at 239 tests; regression harness at 51 canonical / 0 mismatch / 2 baseline parse failures across the local corpus.
